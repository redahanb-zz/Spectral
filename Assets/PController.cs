...
public class PController : MonoBehaviour {
	...
	float agentSpeedMult = 1;

	Transform currentBlendSurface;
	...
	void GetInput(){
		...
		if(Input.GetMouseButtonDown(0) || Input.GetMouseButtonDown(1)){
			...
			if (Physics.Raycast(ray, out rayHit, 100f)){
				...
				case "Blend Surface" :
					//print("Blend Surface");
					print(rayHit.transform + " :: " +currentBlendSurface);
...
//			print ("Wall color: " + rayHit.transform.GetComponent<Renderer>().material.color.b);
	...
	void SetRunSpeed(){
		...
		currentSpeed = Mathf.Lerp(currentSpeed, targetSpeed, myTime * 5);
